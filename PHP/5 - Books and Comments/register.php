...
if(isset($_SESSION['logged'])==true)
{
   ...
   exit;
    ...
    if($_POST){
        ...
        if(strlen($name)<3){
            echo "The username must be more than 3 symbols.</br>";
            $error=true;
            }
        if(strlen($passw1)<3){
            echo "The password must be more than 3 symbols.</br>";
            $error=true;
            }
        if($passw1!=$passw2){
...
?>
    <form method = "POST">
        <div>Username: <input type = "text" name = "user"/></div>(min 3 characters)
        <div>Password:  <input type="password" name = "pass1"/></div>(min 3 characters)
